import { Card } from "../app/components/Deck/services/types";
 ...
 */
export const sortCards = (cards: Card[], rotationCard: string): Card[] => {
  ...
  return cards.sort((a: Card, b: Card) => {
    const [naipeA, numberA] = getValuesFromCard(a.code);
    const [naipeB, numberB] = getValuesFromCard(b.code);
 ...
 */
const sortFullHouseCards = (cards: Card[]): Card[] => {
  return cards.sort((a: Card, b: Card) => {
    const [, card1Number] = getValuesFromCard(a.code);
    const [, card2Number] = getValuesFromCard(b.code);
 ...
 */
export const isFullHouseCombinationsPossible = (cards: Card[]): boolean => {
  /**
  ...
  if (cardsMap.size === 2) {
    ...
    return (
      (card1Value % 2 === 0 && card2Value % 3 === 0) ||
      (card1Value % 3 === 0 && card2Value % 2 === 0)
    );
  }
 ...
 */
const numberOfOccurrencesInMap = (cards: Card[]): Map<string, number> => {
  const valuesMap = new Map<string, number>();

  cards.forEach((card: Card) => {
    const [, cardNumber] = getValuesFromCard(card.code);
 ...
 */
export const findFullHouseCombinations = (cards: Card[]) => {
